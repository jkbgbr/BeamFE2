...
class Material_Test(unittest.TestCase):
    ...
    def test_Crosssection(self):
        ...
        args = []
        kwargslist = []
        kwargslist.append({'A': 0, 'I': 1})  # A zero
        kwargslist.append({'A': 10, 'I': -1})  # I negative
        kwargslist.append({'I': -1})  # A is None
        kwargslist.append({'A': 1})  # I is None
        for kw in kwargslist:
            self.assertRaises(AssertionError, CustomCrosssection, *args, **kw)

        cs = CustomCrosssection(A=10, I=35)
